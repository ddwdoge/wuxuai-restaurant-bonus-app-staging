...
begin
  ...
  summary as (
    select jsonb_build_object(
      ...
      'active_trials', count(*) filter (
        where subscription_exists
          and subscription_status = 'trialing'
          and (trial_ends_at is null or trial_ends_at >= now())
      ),
      'expired_trials', count(*) filter (
        where subscription_exists
          and subscription_status = 'trialing'
          and trial_ends_at < now()
      ),
      'active_subscriptions', count(*) filter (
        where subscription_exists
          and subscription_status = 'active'
      ),
      'open_payments', count(*) filter (
        where subscription_exists
          and (payment_status in ('pending', 'failed') or subscription_status in ('past_due', 'unpaid'))
      ),
